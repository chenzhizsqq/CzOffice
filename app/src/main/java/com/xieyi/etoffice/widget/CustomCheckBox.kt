...
class CustomCheckBox @JvmOverloads constructor(
    ...
    private fun initAttr(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        //默认的选中颜色
        val defaultCheckCircleColor = Color.argb(255, 254, 201, 77)
        //默认的未选中颜色
        val defaultUncheckCircleColor = Color.argb(255, 234, 234, 234)
        //默认选中的钩子颜色
        val defaultCheckHookColor = Color.argb(255, 53, 40, 33)
        //默认未选中的钩子颜色
        val defaultUncheckHookColor = Color.argb(255, 255, 255, 255)
        //默认风格
        ...
        if (attrs != null) {
            ...
            mCheckCircleColor = array.getColor(
                R.styleable.CustomCheckBox_hcb_check_circle_color,
                defaultCheckCircleColor
            )
            mUncheckCircleColor = array.getColor(
                R.styleable.CustomCheckBox_hcb_uncheck_circle_color,
                defaultUncheckCircleColor
            )
            mCheckHookColor = array.getColor(
                R.styleable.CustomCheckBox_hcb_check_hook_color,
                defaultCheckHookColor
            )
            mUncheckHookColor = array.getColor(
                R.styleable.CustomCheckBox_hcb_uncheck_hook_color,
                defaultUncheckHookColor
            )
            style = array.getInt(R.styleable.CustomCheckBox_hcb_style, defaultStyle)
            isCheck = array.getBoolean(R.styleable.CustomCheckBox_hcb_is_check, false)
            mLineWidth =
                array.getDimension(R.styleable.CustomCheckBox_hcb_line_width, defaultLineWidth)
            array.recycle()
        } else {
    ...
    public fun setCheck(check: Boolean): CustomCheckBox {
        isCheck = check
    ...
    private open inner class BaseStyleStrategy {
         ...
         * 画圆形背景
         */
        open fun drawCircleBg(canvas: Canvas) {
            //设置背景圆的颜色
            if (isCheck) {
         ...
         * 画钩子
         */
        open fun drawHook(canvas: Canvas) {
            //设置钩子的颜色
            if (isCheck) {
    ...
    private inner class HollowOutStyleStrategy : BaseStyleStrategy() {
        ...
        override fun drawHook(canvas: Canvas) {
            //镂空风格，选中时，才画钩子
            if (isCheck) {
                //设置混合模式
